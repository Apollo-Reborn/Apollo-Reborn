...
tail -5 changelog.md
git --no-pager diff control changelog.md
```

Confirm a fresh empty `## [Unreleased]` section sits above the new release entry, and that the footer has a new `[vX.Y.Z]: ...compare/<prev>...vX.Y.Z` link. Show the diff to the user. **Do not commit or tag** — the user does that.
...
- Don't bump the version in any other file — only `control`. `src/Version.h` is auto-generated by the Makefile.
